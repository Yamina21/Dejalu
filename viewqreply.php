...
$k=1;
$f=1;
$user_id = $_GET['userrrqid'];

  $quoteid=$_GET['quoterid'];
 

        $fuserid= $_GET['ubrq'];
		
		$stmtt  =  $con->prepare("UPDATE qreplynotification SET status = 'qreplyread' WHERE UserID = $user_id AND BookID= $quoteid");
	    $stmtt->execute();
 ...
 <?php 
  ...
  <a type="button" name="btn_moreqreplies" data-qreplycid="<?php echo $reply["id"]; ?>" data-qcrid="<?php echo $comment["id"]; ?>"  data-qnumerr="<?php echo $f; ?>"  id="btn_moreqreplies"  >
			...
			<?php $k++;}}?>
					...
					<?php if ($result == 1 ) {echo'';}else{if($result == 0){ echo'Be the first to comment';}else{?>	<div id="<?php echo 'qremove_row'.$i?>"> 
