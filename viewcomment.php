...
$user_id = $_GET['userrid'];
  ...
  $bookid=$_GET['bookvid'];
	 ...
	 $stmttt  =  $con->prepare("UPDATE commentnotification SET status = 'cread' WHERE UserID = $user_id AND fUserID= $fuserid AND BookID= $bookid");
	 $stmttt->execute();
		$stmt = $con->prepare(" SELECT * FROM books INNER JOIN users   ON books.UserID = users.UserID 
 								 Where books.UserID = $user_id AND books.BookID=$bookid");
 ...
 <div class="row">
   <div class="pro col-xs-9">
	  ...
	  <?php if (empty($row['UserPic'])) {
		...
		else {	echo "<img class='img-circle  img-thumbnail img-responsive' src='uploads/User/" . $row['UserPic'] . "'> </img>";}?><span class="name col-xs-12"> <?php echo $comment['UserName'];?></span>
...
<?php 
 ...
 <a href="profile.php?userid=<?php echo $result['UserID'] ?>"> <?php if (empty($result['UserPic'])) {
		...
		<form class="clearfix col-xs-11 col-xs-offset-1 " action="index.php" method="post" id="comment_form" data-numer="<?php echo $i; ?>">
 				...
 				<textarea dir="auto" name="comment_text" id="<?php echo 'comment_text'.$i?>" class="form-control " onkeypress="preventMoving(event);" cols="30" rows="1" placeholder="Add a public comment..." required></textarea>
  			</form>
...
<form class="reply-form" action="index.php" method="post" id="reply_form" data-numereply="<?php echo $f; ?>">
 ...
 <textarea dir="auto" dir="auto" name="reply_text" id="<?php echo 'reply_text'.$f?>" class="form-control " onkeypress="preventMoving(event);" cols="30" rows="1" placeholder="Add a reply..." required></textarea>
 </form>
